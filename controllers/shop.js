    ...
    .then(products => {
      ...
      .then(user_cart => {
          res.render('newDesign/product', {
            ...
            wishlist: user_cart.wishlist,
            categories,
            resultInfo: `<span>${(ITEMS_PER_PAGE < totalItems ? `${(page - 1) * ITEMS_PER_PAGE + 1}-${ITEMS_PER_PAGE} of ` : '' ) + `${totalItems} result` + (totalItems > 1 ? 's' : '') + (param_1_value ? ` for </span><span> </span><span class="a-color-state a-text-bold"><q>${param_1_value}</q></span>` : '</span>')}`,
            pageTitle: 'Product',
            path: '/product',
            currentPage: page,
            hasNextPage: ITEMS_PER_PAGE * page < totalItems,
