...
exports.formatResultInfo = (param_1_value, ITEMS_PER_PAGE, totalItems, page) => {
    return param_1_value && `<span>${(ITEMS_PER_PAGE < totalItems ? `${(page - 1) * ITEMS_PER_PAGE + 1}-${ITEMS_PER_PAGE} of ` : '' ) + `${totalItems} result` + (totalItems > 1 ? 's' : '') + (param_1_value ? ` for </span><span> </span><span class="a-color-state a-text-bold"><q>${param_1_value.replace('00 ','00+').replace('-',' - ')}</q></span>` : '</span>')}`;
}